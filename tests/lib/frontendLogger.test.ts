// 双运行时适配（fork）：上游经 @tauri-apps/plugin-log 写日志，本 fork 统一走
// adapter invoke("log_frontend_error")（桌面 Tauri command / Web POST 路由），
// 后端 logging::append_frontend_error 落盘 frontend.log。mock 保持 [message]
// 形参形状，便于与上游断言逐条对齐。
const writeErrorLog = vi.hoisted(() =>
  vi.fn<(message: string) => Promise<void>>(() => Promise.resolve()),
);
const invokeMock = vi.hoisted(() =>
  vi.fn((command: string, args?: Record<string, unknown>) => {
    if (command === "log_frontend_error") {
      return writeErrorLog(String(args?.message ?? ""));
    }
...
describe("frontendLogger", () => {
  ...
  it("writes bounded, redacted errors through the backend log command", () => {
    ...
    // fork：统一出口是 adapter invoke 的 log_frontend_error command。
    expect(invokeMock).toHaveBeenCalledWith("log_frontend_error", { message });
  });
