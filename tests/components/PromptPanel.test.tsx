...
const mocks = vi.hoisted(() => ({
  state: {
    ...
    >,
    loading: false,
    currentFileContent: "# Current prompt content" as string | null,
  },
  reload: vi.fn(),
  getReload: vi.fn(),
  savePrompt: vi.fn(),
  ...
  useTranslation: () => ({
    t: (key: string, options?: Record<string, unknown>) => {
      if (key === "prompts.currentFile") {
        return `prompts.currentFile:${String(options?.filename ?? "")}`;
      }
...
describe("PromptPanel", () => {
  beforeEach(() => {
    ...
    mocks.state.currentFileContent = "# Current prompt content";
  ...
  it("exposes openAdd through the panel handle", async () => {
    ...
    render(
      <PromptPanel ref={ref} open onOpenChange={vi.fn()} appId="claude" />,
    );
    await waitForPanelReady();
