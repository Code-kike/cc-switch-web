...
/**
 * Managed-account selection behaviour ported from upstream `a2e22f33`
 * (tests/components/CodexOAuthSection.test.tsx, +380).
 *
 * It lives in a scope-suffixed file rather than in
 * `tests/components/CodexOAuthSection.test.tsx` because that fork suite replaces
 * `@/components/ui/select` (and Button/Label/Badge/Switch plus `react-i18next`)
 * with plain stubs, while these cases assert real Radix Select semantics:
 * `combobox`/`option`/`listbox` roles, a disabled trigger, option ordering,
 * dividers and content sizing. `vi.mock` is file-scoped, so the two mocking
 * strategies cannot coexist in one file.
 */
...
describe("CodexOAuthSection managed account selection", () => {
