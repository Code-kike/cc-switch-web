...
describe("UnifiedSkillsPanel", () => {
  ...
  it("defaults OpenClaw imports to enabled when discovered there", async () => {
    const ref = createRef<UnifiedSkillsPanelHandle>();
    unmanagedSkillsMock = [
      {
    ...
    await screen.findByText("OpenClaw Skill");
    fireEvent.click(
  ...
  it("formats structured ZIP install errors into user-facing messages", async () => {
    const ref = createRef<UnifiedSkillsPanelHandle>();
    const zipFile = new File(["zip"], "skills.zip", {
      type: "application/zip",
    });
  ...
  it("falls back to a skills-specific toggle error without detail", async () => {
  ...
  it("reports partial bulk-toggle failures", async () => {
    ...
    await waitFor(() => {
      expect(toastErrorMock).toHaveBeenCalledWith("common.bulkToggleFailed", {
        description: "second-id: permission denied",
      });
    });
  });
  ...
  it("waits for an explicit backup refresh before reporting deletion failure", async () => {
    ...
    await waitFor(() => {
      ...
      expect(toastErrorMock).toHaveBeenCalledWith(
        ...
        { description: "common.error", duration: 4000 },
      );
    });
    expect(toastSuccessMock).not.toHaveBeenCalled();
    expect(
