...
export function WebdavSyncSection({
  ...
  const handleS3Test = useCallback(async () => {
    ...
    try {
      ...
      toast.error(formatError(error, "settings.s3Sync.testFailed"));
    } finally {
      setS3ActionState("idle");
    }
  }, [buildS3Settings, formatError, s3SecretTouched, t]);
  ...
  const handleS3Save = useCallback(async () => {
    ...
    try {
      ...
      toast.error(formatError(error, "settings.s3Sync.saveFailed"));
      setS3ActionState("idle");
      return;
    }
    ...
    try {
      ...
      toast.warning(formatError(error, "settings.s3Sync.saveAndTestFailed"));
    } finally {
      setS3ActionState("idle");
    }
  }, [buildS3Settings, formatError, s3SecretTouched, queryClient, t]);
  ...
  const handleS3UploadConfirm = useCallback(async () => {
    ...
    try {
      ...
      toast.error(formatError(error, "settings.s3Sync.uploadFailed"));
    } finally {
      setS3ActionState("idle");
    }
  }, [closeS3Dialog, formatError, s3Dirty, queryClient, t]);
  ...
  const handleS3DownloadClick = useCallback(async () => {
    ...
    try {
      ...
      toast.error(formatError(error, "settings.s3Sync.downloadFailed"));
    } finally {
      setS3ActionState("idle");
    }
  }, [formatError, s3Dirty, t]);
  ...
  const handleMutualExclusionConfirm = useCallback(async () => {
    ...
    try {
      ...
      toast.error(formatError(error, "settings.s3Sync.mutualExclusionFailed"));
    }
