...
export interface DeepLinkImportDialogHandle {
  openManualImport(initialUrl?: string): void;
}

export const DeepLinkImportDialog = forwardRef<DeepLinkImportDialogHandle>(
  function DeepLinkImportDialog(_props, ref) {
    ...
    const getErrorDescription = useCallback(
      (error: unknown) => extractErrorMessage(error) || t("common.unknown"),
      [t],
    );

    const resolveRequest = useCallback(
      async (nextRequest: DeepLinkImportRequest) => {
        if (!nextRequest.config && !nextRequest.configUrl) {
          return nextRequest;
        }

        try {
          return await deeplinkApi.mergeDeeplinkConfig(nextRequest);
        } catch (error) {
    ...
    useEffect(() => {
      ...
      const unlistenImport = listen<DeepLinkImportRequest>(
        "deeplink-import",
        async (event) => {
          setRequest(await resolveRequest(event.payload));
          setIsOpen(true);
        },
      );
    ...
    const handleParseManualUrl = useCallback(async () => {
      ...
      setIsParsing(true);
      try {
        const parsedRequest = await deeplinkApi.parseDeeplink(trimmedUrl);
        setRequest(await resolveRequest(parsedRequest));
      } catch (error) {
    ...
    }, [getErrorDescription, manualUrl, resolveRequest, t]);
          ...
          {!request ? (
            <>
              <DialogHeader className="text-left sm:text-left">
                <DialogTitle>{t("deeplink.pasteImport")}</DialogTitle>
                <DialogDescription>
                  {t("deeplink.pasteImportDescription")}
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="space-y-4 px-8 py-4">
                <Textarea
                  ...
                  placeholder={t("deeplink.pasteImportPlaceholder")}
                  className="min-h-28 font-mono text-xs"
                />
                <p className="text-xs text-muted-foreground">
                  {t("deeplink.pasteImportHint")}
                </p>
              </div>
              ...
              <DialogFooter>
                ...
                <Button
                  onClick={() => void handleParseManualUrl()}
                  disabled={isParsing || manualUrl.trim().length === 0}
                >
                  {isParsing
                    ? t("deeplink.parsing")
                    : t("deeplink.parseAction")}
                </Button>
              </DialogFooter>
            </>
          ) : (
...
DeepLinkImportDialog.displayName = "DeepLinkImportDialog";
