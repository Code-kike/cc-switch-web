...
export function reportFrontendError(
  ...
  // 双运行时统一出口：桌面走 Tauri command，Web 走 POST /api/system/
  // log_frontend_error，两端都汇聚到后端 logging::append_frontend_error
  // 落盘 frontend.log。上报失败绝不能再触发 console.error 或未处理
  // Promise（测试环境/服务不可达时会形成错误循环）。
  try {
    void invoke("log_frontend_error", { message }).catch(() => undefined);
  } catch {
    // 忽略同步失败：日志上报永远不能反向拖垮界面。
  }
}
