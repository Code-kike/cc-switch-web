...
// 深拷贝函数。当前 fork 保留内联实现；与上游 deepClone fallback 一样，必须
// 跳过原型链元键，否则给 `clonedObj["__proto__"]` 赋值会替换克隆体自身的原型。
...
//   setCodexRemoteCompaction, setCodexTopLevelInt,
...
// 仅修改已存在的条目，避免给未使用 bearer-token 模式的配置主动新增字段。
...
export const setCodexModelName = (
  ...
  const escapedModel = tomlBasicString(trimmed);
  const replacementLine = `model = ${escapedModel}`;
  if (modelLineIndex !== -1) {
    const existingLine = lines[modelLineIndex];
    lines[modelLineIndex] = TOML_MODEL_REPLACE_PATTERN.test(existingLine)
      ? existingLine.replace(TOML_MODEL_REPLACE_PATTERN, `$1${escapedModel}$2`)
      : replacementLine;
    return finalizeTomlText(lines);
  }
