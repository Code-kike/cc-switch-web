...
**アカウント未バインドの公式カード**についてはおすすめしません。CC Switch はローカルルーティング有効中、マネージドアカウントがバインドされていない公式プロバイダーへの切り替えをブロックします。プロキシ経由で公式 API にアクセスすると、アカウントリスクが発生する可能性があるためです。Claude の公式プロバイダーは接管中は常にブロックされます。

例外は**マネージド ChatGPT アカウントをバインドした Codex 公式カード**のみです。この場合の認証情報は CC Switch がサーバー側で解決して注入するため、接管中でも切り替えが許可されます。ルーティングは引き続き主にサードパーティ、集約サービス、またはプロトコル変換のための機能です。
