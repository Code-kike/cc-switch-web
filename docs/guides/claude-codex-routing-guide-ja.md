...
ログイン認証情報は `~/.cc-switch/codex_oauth_auth.json` に保存され、token は有効期限前に自動的に更新されます。ここで説明する Claude 側の `codex_oauth` プリセット経路は **`~/.codex/` には書き込みません**ので、Codex CLI 自身のログインとは互いに影響しません。

> 例外: **Codex** タブでマネージドアカウントをバインドした公式カードを使う場合、CC Switch は選択されたアカウントの認証バンドル（`id_token` を含む）を Codex の `~/.codex/auth.json` に書き込み、Codex CLI とローカルルーティングが同じアカウントを参照できるようにします。認証情報の保存先自体は `~/.cc-switch/codex_oauth_auth.json` のままです。
...
まずルーティングページで `Claude Code` のルーティングスイッチをオフにし、その後で公式プロバイダーに切り替えてください——CC Switch は引き継ぎ前の live 設定に戻し、公式ログインの認証情報は全工程を通じて影響を受けません。順序は重要です：引き継ぎが有効なままでは Claude の公式プロバイダーへの切り替えは拒否されます。戻した後も同様にターミナルセッションを開き直す必要があります。
