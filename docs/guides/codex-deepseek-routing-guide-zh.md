...
不建议对**未绑定账号的官方卡**这样做。CC Switch 会在本地路由接管模式下阻止切到未绑定托管账号的官方供应商，因为用代理访问官方 API 可能带来账号风险；Claude 官方供应商在接管模式下一律被阻止。

唯一的例外是**绑定了托管 ChatGPT 账号的 Codex 官方卡**：此时凭据由 CC Switch 在服务端解析并注入，因此接管模式下允许切换。路由仍主要用于第三方、聚合或协议转换场景。
