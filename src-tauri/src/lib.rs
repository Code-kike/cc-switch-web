...
pub fn run() {
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // 否则表现可能只是“应用打不开/闪退”。
            //
            // 数据库版本过新时，必须先于 create_tables()/schema migration 做预检，
            // 避免旧应用对读不懂的新库执行任何 DDL/写入。
            match crate::database::Database::stored_user_version_exceeds_supported(&db_path) {
                Ok(Some(version)) => {
                    log::warn!("数据库版本过新（v{version}），进入恢复界面");
            ...
            // ============================================================
            //
            // 桌面与 Web 两端共享的启动后导入流程（Skills 仓库 / SSOT 迁移、
            // live 配置导入 + 官方预设、OpenCode/OpenClaw/Hermes、OMO、MCP、
            // 提示词）抽取到 `bootstrap::run_post_db_bootstrap`，由两端在创建
            // 数据库后调用，保持 Web 与桌面行为一致（audit F6）。各步骤幂等。
            bootstrap::run_post_db_bootstrap(&app_state);
