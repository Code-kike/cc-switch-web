...
impl<'a> UsageLogger<'a> {
    ...
    pub fn log_request(&self, log: &RequestLog) -> Result<(), AppError> {
        ...
        // L29（跨源去重，非数据丢失）+ c9ac6efd（幂等写入）：
        // `session:{message_id}` 是代理观察与 session 日志共享的主键，代理观察
        // 更权威（真实延迟/状态/费用），因此只在覆盖 session_log 行时使用
        // INSERT OR REPLACE；语义相同的代理重放已在上方提前返回，同 id 不同
        // 语义的响应落在确定性 SHA-256 collision fallback 主键上。
