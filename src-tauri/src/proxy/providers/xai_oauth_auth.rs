...
impl XaiOAuthManager {
    ...
    fn write_store_atomic(&self, content: &str) -> Result<(), XaiOAuthError> {
        crate::config::write_text_file(&self.storage_path, content)
...
mod tests {
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn account_store_rejects_final_symlink_without_replacing_it() {
        use std::os::unix::fs::symlink;

        let data_dir = tempfile::tempdir().unwrap();
        let target = data_dir.path().join("external-auth.json");
        let storage = data_dir.path().join("xai_oauth_auth.json");
        fs::write(&target, b"{}").unwrap();
        symlink(&target, &storage).unwrap();
