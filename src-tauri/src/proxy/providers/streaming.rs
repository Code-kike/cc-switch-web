...
pub fn create_anthropic_sse_stream<E: std::error::Error + Send + 'static>(
    ...
    async_stream::stream! {
        ...
        // Some upstream providers can send multiple chunks with finish_reason.
        // Anthropic SSE permits one terminal message_delta, so delay it until
        // [DONE] or stream end and keep replacing its usage with later chunks.
                            ...
                            if let Some(data) = strip_sse_field(l, "data") {
                                ...
                                if let Ok(chunk) = serde_json::from_str::<OpenAIStreamChunk>(data) {
                                    ...
                                    if let Some(choice) = chunk.choices.first() {
                                        ...
                                        // 处理 finish_reason。
                                        // Some providers send more than one finish_reason chunk,
                                        // often with usage arriving later. Cache the terminal
                                        // message_delta and emit it once at [DONE] or stream end.
        ...
        if !stream_ended_with_error {
            let emitted_pending_message_delta =
                if let Some((stop_reason, usage_json)) = pending_message_delta.take() {
                    let event = build_message_delta_event(stop_reason, usage_json);
                    let sse_data = format!("event: message_delta\ndata: {}\n\n",
                        serde_json::to_string(&event).unwrap_or_default());
...
mod tests {
    ...
    #[tokio::test]
    async fn test_empty_tool_calls_do_not_split_reasoning_block() {
        let input = concat!(
            "data: {\"id\":\"chatcmpl_reasoning\",\"model\":\"minimax\",\"choices\":[{\"delta\":{\"role\":\"assistant\",\"reasoning_content\":\"用户\",\"tool_calls\":[]}}]}\n\n",
            "data: {\"id\":\"chatcmpl_reasoning\",\"model\":\"minimax\",\"choices\":[{\"delta\":{\"reasoning_content\":\"用\",\"tool_calls\":[]}}]}\n\n",
            "data: {\"id\":\"chatcmpl_reasoning\",\"model\":\"minimax\",\"choices\":[{\"delta\":{\"reasoning_content\":\"中文\",\"tool_calls\":[]}}]}\n\n",
            "data: {\"id\":\"chatcmpl_reasoning\",\"model\":\"minimax\",\"choices\":[{\"delta\":{},\"finish_reason\":\"stop\"}]}\n\n",
            "data: [DONE]\n\n"
        );
        ...
        assert_eq!(
            thinking_starts, 1,
            "empty tool_calls arrays should not restart the active thinking block"
        );
        assert_eq!(
            block_stops, 1,
            "thinking block should stop once at normal stream finalization"
        );
        assert_eq!(thinking_text, "用户用中文");
    }
