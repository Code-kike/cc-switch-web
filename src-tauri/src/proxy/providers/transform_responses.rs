...
mod tests {
    ...
    #[test]
    fn test_hosted_web_search_non_positive_max_uses_fails_closed() {
        // 钉桩 fail-closed 矩阵最后一行：max_uses 必须是正整数。0/负数/小数/字符串
        // 都不能被静默当成"无限制"（那会绕过调用方设定的搜索预算）。
        for max_uses in [json!(0), json!(-1), json!(1.5), json!("3"), json!([1])] {
            let input = json!({
                "model": "gpt-5",
                "messages": [{"role": "user", "content": "Search"}],
                "tools": [{
            ...
            let error = anthropic_to_responses(input, None, false, false).unwrap_err();
            // 断言变体 + 逐字消息（Display 前缀在本 fork 已本地化，不参与断言）。
            let ProxyError::InvalidRequest(message) = error else {
                panic!("non-positive max_uses must be rejected as InvalidRequest");
            };
            assert_eq!(
                message,
                "Anthropic WebSearch max_uses must be a positive integer"
            );
        }

        // 显式 null 与字段缺失等价：不设限制，且不得报错。
        let input = json!({
            "model": "gpt-5",
            "messages": [{"role": "user", "content": "Search"}],
            "tools": [{
