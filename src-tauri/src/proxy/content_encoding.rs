...
//!
//! Reqwest auto-decompression is disabled so the proxy can preserve the client's
//! original accept-encoding header. Request and response paths therefore share
//! this explicit decoding logic.
    ...
    match coding {
        ...
        "deflate" => {
            ...
            match read_with_output_limit(zlib, max_output_bytes) {
                ...
                // Never downgrade a confirmed over-budget zlib stream into the raw
                // fallback: callers distinguish TooLarge from corrupt input.
                Err(error @ DecompressError::TooLarge { .. }) => Err(error),
                Err(zlib_err) => {
                    log::debug!("deflate zlib decode failed ({zlib_err}); trying raw deflate");
...
mod tests {
    ...
    #[test]
    fn decompress_body_with_limit_rejects_deflate_bomb_without_raw_fallback() {
        ...
        let result = decompress_body_with_limit("deflate", &compressed, 1024 * 1024);
    ...
    fn is_supported_content_encoding_matches_decompressable() {
        for encoding in [
            ...
            assert!(is_supported_content_encoding(encoding), "{encoding}");
        }
