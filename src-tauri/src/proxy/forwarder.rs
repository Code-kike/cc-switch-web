...
impl RequestForwarder {
    ...
    /// last_request_at，并将 active_connections guard 随成功响应向后传递。
    #[allow(clippy::too_many_arguments)]
    pub async fn forward_with_retry(
        ...
        let connection_guard = ActiveConnectionGuard::acquire(self.status.clone()).await;
        ...
        match result {
            Ok(mut result) => {
                result.connection_guard = Some(connection_guard);
                Ok(result)
            }
            Err(error) => Err(error),
        }
    ...
    async fn forward_with_retry_inner(
        ...
        for provider in providers.iter() {
            // 整流器重试标记按 provider 独立持有，避免首家 provider 的整流结果短路后续故障转移。
