...
/// `/v1/models/gemini-1.5-flash`, and `/gemini/v1beta/models/<model>:streamGenerateContent`.
...
mod tests {
    ...
    #[test]
    fn extract_model_with_query_string_without_action() {
        assert_eq!(
            extract_gemini_model_from_path("/v1/models/gemini-pro?key=abc").as_deref(),
            Some("gemini-pro"),
        );
    }
