...
mod tests {
    ...
    #[test]
    fn pi_is_a_supported_session_provider() {
        let err = load_messages("pi", "/tmp/definitely-not-a-pi-session.jsonl")
            .expect_err("missing pi session should not succeed");

        assert!(
            !err.contains("Unsupported provider"),
            "pi must be dispatched by session_manager: {err}"
        );
        assert!(
            err.contains("Session root not found")
                || err.contains("session source not found")
                || err.contains("outside provider roots"),
            "unexpected pi load error: {err}"
        );
    }
