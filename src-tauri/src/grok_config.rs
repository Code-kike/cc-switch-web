...
///
/// Grok Official uses the CLI's native xAI OAuth login and therefore has no
/// custom model table. Live snapshots and backup restoration use this weaker
/// validation; custom providers still go through `validate_config_toml`.
...
/// Whether a live config document represents the CLI's official login state.
///
/// Official mode may contain unrelated sections such as MCP projections, but
/// it must contain neither `[models]` nor `[model.*]`. A malformed or partial
/// custom model document is deliberately not treated as official.
...
/// Read live `config.toml` as a snapshot, accepting the official no-model state.
...
/// Raw live-file writer used by snapshot restoration and proxy lifecycle code.
/// Provider-owned custom configuration is shape-validated by
/// `write_grok_provider_live`; this boundary only requires valid TOML syntax.
pub fn write_grok_live_settings(settings: &Value) -> Result<(), AppError> {
    ...
    validate_config_toml_syntax(config)?;
    write_text_file_managed(&get_grok_config_path(), config)
}
...
"#
    ...
    fn syntax_validation_accepts_official_snapshots() {
        ...
        validate_config_toml_syntax("[mcp_servers.echo]\ncommand = \"echo\"\n")
            .expect("official snapshot with MCP projection");
        assert!(validate_config_toml_syntax("not = [valid").is_err());
    }

    #[test]
    fn detects_only_valid_no_model_documents_as_official() {
    ...
    #[test]
    #[serial]
    fn official_provider_roundtrips_without_touching_native_credentials() {
        let temp = TempDir::new().expect("temp dir");
        let original_test_home = std::env::var_os("CC_SWITCH_TEST_HOME");
        std::env::set_var("CC_SWITCH_TEST_HOME", temp.path());

        let credential_path = get_grok_config_dir().join("credentials.json");
        fs::create_dir_all(get_grok_config_dir()).expect("create Grok directory");
        fs::write(&credential_path, "native-oauth-state").expect("seed native credential file");
        ...
        assert_eq!(
            fs::read_to_string(&credential_path).expect("read native credentials"),
            "native-oauth-state"
        );
        ...
        assert_eq!(
            read_grok_live_settings()
                .expect("official-mode live is readable")
                .get("config")
                .and_then(Value::as_str),
            Some(official_live)
        );
