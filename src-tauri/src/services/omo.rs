use crate::config::{atomic_write_managed, get_home_dir, write_json_file_managed_with_contents};
...
type OptionalConfigFileVersions = Option<(ConfigFileSnapshot, Vec<u8>)>;
...
impl UnifiedConfigDocument {
    ...
    fn save(self) -> Result<Vec<u8>, AppError> {
        ...
        atomic_write_managed(&self.path, &next_contents)?;
        Ok(next_contents)
    }
}
...
impl OmoService {
    ...
    fn remove_unified_config_section(path: &Path) -> Result<OptionalConfigFileVersions, AppError> {
        let previous_snapshot = Self::snapshot_config_file(path)?.ok_or_else(|| {
            AppError::Config(format!("OMO config disappeared: {}", path.display()))
        })?;
        ...
        Ok(Some((previous_snapshot, expected_contents)))
    }
    ...
    fn snapshot_config_file(path: &Path) -> Result<Option<ConfigFileSnapshot>, AppError> {
        ...
        let symlink_target = if metadata.file_type().is_symlink() {
            Some(std::fs::read_link(path).map_err(|err| AppError::io(path, err))?)
        } else {
            None
        };
        let contents = std::fs::read(path).map_err(|err| AppError::io(path, err))?;
        Ok(Some(ConfigFileSnapshot {
            contents,
            symlink_target,
        }))
    }
    ...
    fn restore_config_file(
        path: &Path,
        snapshot: Option<&ConfigFileSnapshot>,
    ) -> Result<(), AppError> {
        match snapshot {
            Some(snapshot) => {
                if let Some(target) = snapshot.symlink_target.as_deref() {
                    match std::fs::symlink_metadata(path) {
                        Ok(metadata) if metadata.file_type().is_symlink() => {
                            let current_target =
                                std::fs::read_link(path).map_err(|err| AppError::io(path, err))?;
                            if current_target != target {
                                return Err(AppError::Config(format!(
                                    "Config symlink changed; refusing to roll back {}",
                                    path.display()
                                )));
                            }
                        }
                        Ok(_) => {
                            return Err(AppError::Config(format!(
                                "Config path is no longer a symlink; refusing to roll back {}",
                                path.display()
                            )));
                        }
                        Err(err) if err.kind() == std::io::ErrorKind::NotFound => {
                            let resolved_target = if target.is_absolute() {
                                ...
                                path.parent()
                                    .ok_or_else(|| {
                                        AppError::Config(format!(
                                            "Config symlink has no parent: {}",
                                            path.display()
                                        ))
                                    })?
                                    .join(target)
                            };
                            let current_target_contents = std::fs::read(&resolved_target)
                                .map_err(|error| AppError::io(&resolved_target, error))?;
                            if current_target_contents != snapshot.contents {
                                return Err(AppError::Config(format!(
                                    "Config symlink target changed; refusing to roll back {}",
                                    path.display()
                                )));
                            }
                            ...
                            Self::create_file_symlink(target, path)
                    ...
                    if let Err(error) = atomic_write_managed(path, &snapshot.contents) {
                        return Err(error);
                    }
                    Ok(())
                } else {
                    crate::config::atomic_write(path, &snapshot.contents)
                }
            }
    ...
    fn write_profile_config(
        ...
        let (previous_contents, expected_contents) = match &location {
            OmoConfigLocation::Unified(path) => {
                let mut document = UnifiedConfigDocument::load(path)?;
                let previous_contents = Self::snapshot_config_file(path)?.ok_or_else(|| {
                    AppError::Config(format!("OMO config disappeared: {}", path.display()))
                })?;
                if previous_contents.contents != document.original_source.as_bytes() {
                    return Err(AppError::Config(
                        "OMO config changed on disk. Please reload and try again.".to_string(),
                    ));
                }
                if document.set_opencode_section(&merged)? {
                    ...
                    (Some(previous_contents), expected_contents)
                } else {
                    (None, None)
                }
            }
            OmoConfigLocation::Legacy(path) => {
                ...
                let expected_contents = Some(write_json_file_managed_with_contents(path, &merged)?);
...
mod tests {
  ...
  "[codex]": {"agents": {"reviewer": {"model": "openai/gpt-5.4"}}}
    ...
    #[cfg(unix)]
    #[test]
    fn test_unified_config_write_preserves_managed_symlink() {
        use std::os::unix::fs::symlink;

        let dir = tempfile::tempdir().unwrap();
        let managed_dir = dir.path().join("managed");
        let config_dir = dir.path().join(".omo");
        std::fs::create_dir_all(&managed_dir).unwrap();
        std::fs::create_dir_all(&config_dir).unwrap();
        let target = managed_dir.join("omo.jsonc");
        let link = config_dir.join("omo.jsonc");
        std::fs::write(
            &target,
            r#"{
  // Keep dotfiles-managed comments.
  "[opencode]": {"agents": {"old": {}}},
  "[codex]": {"agents": {"reviewer": {}}}
}"#,
        )
        .unwrap();
        symlink("../managed/omo.jsonc", &link).unwrap();

        let mut document = UnifiedConfigDocument::load(&link).unwrap();
        document
        ...
        assert!(source.contains("// Keep dotfiles-managed comments."));
        let root: Value = json5::from_str(&source).unwrap();
        assert!(root[OPENCODE_SECTION_KEY]["agents"].get("new").is_some());
        assert!(root.get("[codex]").is_some());
    }
  ...
  "_migrations": ["done"]
    ...
    fn test_rollback_refuses_to_overwrite_newer_contents() {
        ...
        let original_snapshot = ConfigFileSnapshot {
            contents: original.to_vec(),
            symlink_target: None,
        };

        let result = OmoService::restore_config_file_if_unchanged(
            &path,
            Some(written),
    ...
    #[cfg(unix)]
    #[test]
    fn test_rollback_recreates_deleted_managed_symlink_without_replacing_target() {
        use std::os::unix::fs::symlink;

        let dir = tempfile::tempdir().unwrap();
        let target = dir.path().join("managed.jsonc");
        let link = dir.path().join("oh-my-openagent.jsonc");
        let original = br#"{"agents":{"old":{}}}"#;
        std::fs::write(&target, original).unwrap();
        symlink("managed.jsonc", &link).unwrap();
        let snapshot = OmoService::snapshot_config_file(&link)
            .unwrap()
            .expect("managed link snapshot");
        std::fs::remove_file(&link).unwrap();

        OmoService::restore_config_file_if_unchanged(&link, None, Some(&snapshot)).unwrap();
    ...
    #[cfg(unix)]
    #[test]
    fn test_rollback_refuses_deleted_symlink_when_target_changed() {
        use std::os::unix::fs::symlink;

        let dir = tempfile::tempdir().unwrap();
        let target = dir.path().join("managed.jsonc");
        let link = dir.path().join("oh-my-openagent.jsonc");
        std::fs::write(&target, br#"{"agents":{"old":{}}}"#).unwrap();
        symlink("managed.jsonc", &link).unwrap();
        let snapshot = OmoService::snapshot_config_file(&link)
            .unwrap()
            .expect("managed link snapshot");
        std::fs::remove_file(&link).unwrap();
        let concurrent = br#"{"agents":{"newer":{}}}"#;
        std::fs::write(&target, concurrent).unwrap();

        let result = OmoService::restore_config_file_if_unchanged(&link, None, Some(&snapshot));
