//! Web-runtime stub twin of `s3_auto_sync.rs` (mirrors `webdav_auto_sync_web.rs`).
//!
//! The S3 auto-sync background worker is desktop-only (it depends on
//! `tauri::{AppHandle, Emitter}`); web-server deployments get manual S3 sync
//! commands only. This stub keeps the suppression-guard API and the database
//! update-hook callback compiling in the web build as no-ops.
