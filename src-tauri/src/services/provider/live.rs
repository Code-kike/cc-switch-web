...
/// either runtime without an `AppState` field.
///
/// Fail-closed: if the context is not injected yet, a managed Codex Official
/// card errors out of [`apply_codex_managed_oauth_auth`] rather than being
/// written with its stored placeholder credential.
...
pub(crate) fn build_effective_provider_for_live_with_codex_oauth_manager(
    ...
    apply_codex_managed_oauth_auth(app_type, &mut effective_provider, codex_oauth_manager)?;
