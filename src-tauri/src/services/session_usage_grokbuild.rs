...
/// Maximum size of one updates.jsonl file read into memory (50 MiB).
const MAX_GROK_FILE_BYTES: u64 = 50 * 1024 * 1024;
/// Maximum directory depth for session discovery.
const MAX_COLLECT_DEPTH: usize = 16;

/// Recursively collect session files while treating every symlink as hostile.
/// This applies to both directory and file links: following either could escape
/// the Grok session root or create a symlink cycle.
fn collect_files_named(root: &Path, name: &str, files: &mut Vec<PathBuf>, depth: usize) {
    ...
    for entry in entries.flatten() {
        let path = entry.path();
        // DirEntry::metadata() inspects the entry itself, unlike path.metadata(),
        // so symlinks are rejected before is_dir() can follow them.
        ...
        if metadata
            ...
            .unwrap_or(false)
        {
            log::info!("[GROK-SYNC] skipping symlink: {}", path.display());
            continue;
        }
...
fn sync_single_grok_file(db: &Database, file_path: &Path) -> Result<SessionSyncResult, AppError> {
    ...
    if metadata.file_type().is_symlink() {
        log::info!("[GROK-SYNC] skipping symlink file: {}", file_path.display());
        return Ok(SessionSyncResult::default());
    }
    // Reject oversized logs before read_to_string so a hostile or corrupted
    // updates.jsonl cannot exhaust the Web runtime's memory.
...
mod tests {
    ...
    #[test]
    fn oversized_updates_jsonl_is_skipped_before_read() {
        ...
        file.set_len(MAX_GROK_FILE_BYTES + 1)
            .expect("extend oversized fixture");
        drop(file);

        let result = sync_single_grok_file(&db, &path).expect("oversized file is skipped");
    ...
    #[test]
    fn traversal_stops_after_sixteen_levels() {
        let temp = tempdir().expect("tempdir");
        let root = temp.path().join("sessions");
        let mut nested = root.clone();
        for index in 0..(MAX_COLLECT_DEPTH + 1) {
            nested = nested.join(format!("level-{index}"));
            std::fs::create_dir_all(&nested).expect("create nested fixture");
        }
        std::fs::write(nested.join("updates.jsonl"), b"{}\n").expect("write fixture");

        let mut files = Vec::new();
        collect_files_named(&root, "updates.jsonl", &mut files, 0);
        assert!(
            files.is_empty(),
            "a file below the 16-level traversal cap must not be collected"
        );
    }

    #[test]
    fn collector_skips_directory_and_file_symlinks() {
        let temp = tempdir().expect("tempdir");
        let root = temp.path().join("sessions");
        let real = root.join("real");
        std::fs::create_dir_all(&real).expect("create real session dir");
        std::fs::write(real.join("updates.jsonl"), b"{}\n").expect("write real fixture");

        #[cfg(unix)]
        {
            std::os::unix::fs::symlink(&real, root.join("dir-link")).expect("directory symlink");
            std::os::unix::fs::symlink(real.join("updates.jsonl"), root.join("updates.jsonl"))
                .expect("file symlink");
        }
        #[cfg(windows)]
        {
            std::os::windows::fs::symlink_dir(&real, root.join("dir-link"))
