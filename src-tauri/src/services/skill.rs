...
/// Pi Skills 目录中已确认由 CC Switch 部署的副本形态。
///
/// 卸载/迁移等破坏性操作前先用 [`Self::inspect_pi_skill_destination`] 记录快照，
/// 操作后用 [`Self::refresh_pi_skill_destination`] 复核：若用户在期间改动了原生
/// 目录（增删隐藏文件、改类型、换链接目标），复核失败并拒绝覆盖。
#[derive(Debug)]
...
impl SkillService {
    ...
    /// 两个路径是否重叠（一方是另一方的祖先，或经别名解析后重叠）。
    ///
    /// `canonicalize()` 会跟随末级组件，因此对悬空符号链接会失败。分别解析父目录
    /// 再拼回文件名，避免两个应用通过别名根删除同一目录项。
    ...
    ///
    /// 与 [`Self::collect_files_for_hash`] 不同，这里**不跳过隐藏文件**：pi 副本
    /// 的“是否被用户改过”判定必须覆盖 `.env` 等隐藏原生文件，否则用户在原生目录
    /// 里加的私有内容会被静默覆盖。该哈希只对已通过 [`Self::inspect_pi_skill_destination`]
    /// 校验的 pi 目标目录（小型 skill 目录，非任意路径）运行，作用域窄。
    ...
    /// 收集目录下所有条目（含隐藏文件、含符号链接本身，但不跟随符号链接递归）。
    ///
    /// `entry.file_type()` 不跟随符号链接，因此符号链接目录不会被当作目录递归进入，
    /// 与 fork 既有“跳过 symlink 递归”的安全口径一致。
    ...
    /// 检查 Pi 目标目录是否为 CC Switch 部署的副本（符号链接指向 source，或目录树哈希与 source 一致）。
    ///
    /// 返回 `Ok(None)` 表示目标不存在；`Ok(Some(deployment))` 表示已确认由 CC Switch 部署；
    /// `Err` 表示存在同名但内容不同的外部目录，拒绝覆盖或删除。
    ///
    /// 这里**故意跟随符号链接**（`fs::read_link` + `canonicalize`）以判定链接目标是否指向 source：
    /// pi 的 exists=active 规则要求区分“CC Switch 管理的链接”与“用户自建的外部目录”，
    /// 不跟随就无法识别。作用域仅限已校验的 pi 目标路径，不用于任意路径枚举。
...
mod tests {
    ...
    // ========== P3.5: pi skills 安全路径回归测试（移植自上游 84e75ad2） ==========
