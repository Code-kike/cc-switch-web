...
//!
//! 本 fork 未纳入 Claude Desktop 应用（`AppType` = Claude/Codex/Gemini/OpenCode/
//! OpenClaw/Hermes），因此 `ProfileScope` 只有 Claude 与 Codex 两个分组；上游
//! 的 `ClaudeDesktop` 分组在本 fork 无对应 live 文件与切换路径，不予移植。
//! Gemini/OpenCode/OpenClaw/Hermes 暂不建分组（其标签页不渲染切换器），后续任务再扩展。
...
///
/// 本 fork 只支持 Claude Code 与 Codex 两个分组，两者 live 文件零交集
///（`~/.claude` / `~/.codex`），分组切换互不干扰。
...
impl ProfileService {
    ...
    /// 其它应用也没有接管时，建议调用者停止代理服务，以便"本地路由"总开关
    /// 同步显示为关闭。
    pub fn apply(
        state: &AppState,
        profile_id: &str,
...
mod tests {
    ...
    fn test_scope_captured_detects_per_scope_snapshot() {
        ...
        // codex 槽位属于独立的 codex 组
        let mut codex_only = ProfilePayload::default();
        codex_only.providers.codex = Some("c1".into());
        assert!(codex_only.scope_captured(ProfileScope::Codex));
        assert!(!codex_only.scope_captured(ProfileScope::Claude));
    }
    ...
    fn test_scope_serde_and_parse_roundtrip() {
        ...
        // fork 未纳入 Claude Desktop：其 scope 字符串必须被拒
        assert!(ProfileScope::parse("claude-desktop").is_err());
    ...
    fn test_scope_app_grouping() {
        // Claude Code 与 Codex 各自独立成组；组内应用与 for_app 反向映射必须一致
