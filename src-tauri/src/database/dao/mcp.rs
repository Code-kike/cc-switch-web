...
impl Database {
    ...
    pub fn update_mcp_server_app_enabled(
        ...
        app: &AppType,
        enabled: bool,
    ) -> Result<Option<(bool, McpServer)>, AppError> {
        let conn = lock_conn!(self.conn);
        let column = match app {
            ...
            AppType::OpenClaw | AppType::Pi => None,
        };
        ...
        let previous_enabled = previous.apps.is_enabled_for(app);
        ...
        if let Some(column) = column {
            ...
            return Ok(Some((previous_enabled, previous)));
        }

        conn.query_row(
            &format!("{MCP_SERVER_SELECT} WHERE id = ?1"),
            params![id],
            |row| row_to_mcp_server(row).map(|(_, server)| (previous_enabled, server)),
        )
        .optional()
        .map_err(|e| AppError::Database(e.to_string()))
    }
...
mod tests {
    ...
    fn unsupported_mcp_apps_keep_the_existing_noop_semantics() {
        ...
        let (previous_enabled, returned) = db
