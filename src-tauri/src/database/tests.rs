...
fn schema_create_tables_repairs_dev_global_profile_marker() {
    ...
    // 模拟跑过未发布开发版的库：user_version 已是 14（迁移不会再跑），
    // 但 current 标记还是最初 foundation 版本的全局 key。
    conn.execute_batch(
        r#"
...
fn schema_model_pricing_is_seeded_on_init() {
    ...
    for (model_id, display_name, input, output, cache_read, cache_creation) in [
        // DeepSeek V4 自 2026-08-16 起改峰谷双档计价，seed 统一录高峰档
        // （bad9c151）：flash 0.44/1.32/0.014；pro 1.32/3.96/0.044。
        (
            "deepseek-v4-flash",
            "DeepSeek V4 Flash",
            "0.44",
