...
impl Database {
        ...
        F: FnOnce() -> Result<(), AppError>,
    {
        ...
        // preserve_tables（同步路径）的 live 读取改到替换临界区内（持有主库锁、
        // on_staging_ready 之后）进行，以捕获 staging 完成后到达的设备本地写入。
        // 此处不再提前快照本地专用表。
        ...
        Self::ensure_incremental_auto_vacuum_on_conn(&normalized_conn)?;
        ...
        // sqlite_sequence 的 high-water mark 也随数据一起恢复，避免 AUTOINCREMENT
        // 表被恢复后回退到 1，与上游 #6146 的保真要求一致。
        Self::restore_sqlite_sequences(&temp_conn, &normalized_conn, SQL_RESTORE_TABLES)?;
        normalized_conn
            .execute("PRAGMA foreign_keys = ON;", [])
        ...
        // 同步协调点（#6147）：canonical staging 完成、替换主库之前，让调用方
        // 抢全局 sync 锁——避免 staging 与替换之间有设备本地写入漏过同步窗口。
        on_staging_ready()?;

        let backup_file_guard = lock_backup_file_operations()?;
        // preserve_tables（同步路径）必须在 on_staging_ready 之后、持有主库锁时
        // 从 live 读取到 normalized_conn——这才能捕获 staging 完成后、替换前到达
        // 的设备本地写入（c9fe340b 的语义）。早期快照会漏掉这批写入。
        let backup_path = self.replace_main_with_safety_backup(
            &normalized_conn,
            &backup_file_guard,
            preserve_tables,
        )?;
            ...
            let allowed = match ctx.action {
                ...
                AuthAction::Update {
                    ...
                    (table_name == "sqlite_master"
                        ...
                        || (table_name == "sqlite_sequence" && column_name == "seq")
                }
                AuthAction::Delete { table_name } => table_name == "sqlite_sequence",
                AuthAction::Read {
                    table_name,
                    column_name,
                } => {
    ...
    /// canonical-restore 路径：把可信规范化库原子替换主库，并在替换前先做一份
    /// 安全备份。保留 fork 的 `RESTORE_CRITICAL_SECTION_HOOK` 测试钩子——它是
    /// `sql_import_holds_main_lock_across_safety_backup_and_replace` 锁回归的依赖。
    fn replace_main_with_safety_backup(
        ...
        // Hold the main-DB guard across the safety snapshot, preserve-table read,
        // hook, AND the final replacement so concurrent writers stay blocked for
        // the whole critical section (the `sql_import_holds_main_lock_across_safety_backup_and_replace`
        // regression asserts this). The test hook fires inside this window.
        let mut main_conn = lock_conn!(self.conn);
        let backup_path = Self::backup_database_file_from_conn(backup_file_guard, &main_conn, &[])?;
        #[cfg(test)]
        if let Some(hook) = {
        ...
        // Read preserve_tables from live AFTER the hook so writes that arrived
        // during staging (after on_staging_ready fired) are captured into the
        // canonical schema before the replacement overwrites main.
        if !preserve_tables.is_empty() {
            Self::restore_tables(&main_conn, source_conn, preserve_tables)?;
            Self::restore_sqlite_sequences(&main_conn, source_conn, preserve_tables)?;
        }
    ...
    fn validate_imported_schema(conn: &Connection) -> Result<(), AppError> {
        ...
        if !missing.is_empty() {
            ...
            return Err(AppError::localized(
                ...
                format!("导入的 SQL 缺少 CC Switch 必��表：{names}"),
...
#[cfg(test)]
mod tests {
    use super::{lock_backup_file_operations, Database, RESTORE_CRITICAL_SECTION_HOOK};
    use crate::error::AppError;
