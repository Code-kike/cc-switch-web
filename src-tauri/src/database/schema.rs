...
impl Database {
    ...
    pub(crate) fn create_tables_on_conn(conn: &Connection) -> Result<(), AppError> {
        ...
        // pricing_model 保留写入时的计价基准，避免明细 prune 后计费不可审计。
        conn.execute(
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    10 => {
                        log::info!(
                            "迁移数据库从 v10 到 v11（usage rollup 保留请求与计价模型维度）"
                        );
    ...
    fn repair_current_model_pricing(conn: &Connection) -> Result<(), AppError> {
        let pricing_fixes = [
            ...
            // Qwen3.6 Plus cache-read pricing from the vendor list; repair only
            // rows that still carry the earlier zero cache-read seed.
            (
                "qwen3.6-plus",
                "Qwen3.6 Plus",
                "0.325",
