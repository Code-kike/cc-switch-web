...
///
/// 事件 id 形如 `profile_<scope>_<uuid>`；`profile_none_<scope>` 仅清除
/// 对应分组的 current 标记。ProfileScope 只接受 Claude/Codex，因此本 fork
/// 不会创建或解析 Claude Desktop 菜单项。
        ...
        let mut queue = app_state
            ...
            .filter(|item| {
                all_providers
                    ...
                    .is_some_and(|provider| provider.supports_failover(app_type_str))
            })
            .collect::<Vec<_>>();
        if queue.is_empty() {
            let current_id =
            ...
            if !current.supports_failover(app_type_str) {
                return Err(AppError::Message(
...
pub fn create_tray_menu(
    ...
    // 项目列表全应用共享；Claude Code 与 Codex 各自拥有独立 current 指针，
    // 因此托盘按 scope 嵌套同一份列表。
    {
...
mod tests {
    ...
    fn codex_summary_thirty_day_only_still_renders() {
        let quota = make_quota("codex", true, vec![tier(TIER_THIRTY_DAY, 42.0)]);
        let s = format_subscription_summary(&quota).expect("should format");
