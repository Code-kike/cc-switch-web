use crate::config::{atomic_write_managed, write_json_file_managed_with_contents};
...
fn write_opencode_config_to_path_with_contents(
    ...
    if let Some(source) = comment_preserving_source(path, config) {
        let contents = source.into_bytes();
        atomic_write_managed(path, &contents)?;
        log::debug!("OpenCode config written (comment-preserving) to {path:?}");
        return Ok(contents);
    }

    let contents = write_json_file_managed_with_contents(path, config)?;
