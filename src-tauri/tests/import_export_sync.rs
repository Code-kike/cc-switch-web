...
#[test]
fn import_sql_accepts_empty_cc_switch_export_roundtrip() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    let home = ensure_test_home();

    let state = create_test_state().expect("create test state");
    let export_path = home.join("cc-switch-empty-export.sql");
    state
        .db
        .export_sql(&export_path)
    ...
    reset_test_fs();
    let state = create_test_state().expect("create fresh test state");
    state
        .db
        .import_sql(&export_path)
        .expect("empty export import should succeed");
