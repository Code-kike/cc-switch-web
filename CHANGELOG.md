...
Synced product-upstream `farion1231/cc-switch` `v3.19.2..v3.20.0` (selective port, 7 themed batches S1–S7) into this Web-first fork, preserving the standalone Web-server deployment model, dual-runtime parity, the no-auth posture, the disabled desktop updater, and the existing Web hardening. Three large new features (Pi native coding agent, Codex Alpha WebSearch, managed OAuth accounts) are split into dedicated subtasks and land separately. Fork-specific deviations: the `zh-TW` locale and `README_DE` are not restored; the `claudeDesktopProviderPresets` surface and desktop-only README sponsor banner tables remain excluded; the Codex Chat routing stack (`streaming_codex_chat.rs`/`transform_codex_chat.rs` et al.) remains an intentional carry-forward deferral, so upstream commits touching only that stack were skipped as proven-inapplicable; the `36ed280d` i18n labeler glob fix was skipped because the fork has no labeler CI surface (its intent is equivalently covered by the inlined path filters in `c98cc3a9`).
...
See **[release notes ->](docs/release-notes/v3.20.0-en.md)** for the per-batch scope, Web adaptations, carry-forward deferrals, and the three subtask-bound features.
