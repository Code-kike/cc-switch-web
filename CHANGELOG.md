...
Synced product-upstream `farion1231/cc-switch` `v3.19.2..v3.20.0` (selective port) into this Web-first fork, preserving the standalone Web-server deployment model, dual-runtime parity, the no-auth posture, the disabled desktop updater, and the existing Web hardening. This release also lands the three large new features of the cycle: the Pi native coding agent, Codex Alpha WebSearch, and managed OAuth account selection. Fork-specific deviations: the `zh-TW` locale and `README_DE` are not restored; the `claudeDesktopProviderPresets` surface and desktop-only README sponsor banner tables remain excluded; the Codex Chat routing stack (`streaming_codex_chat.rs`/`transform_codex_chat.rs` et al.) remains an intentional carry-forward deferral, so upstream commits touching only that stack were skipped as proven-inapplicable; the `36ed280d` i18n labeler glob fix was skipped because the fork has no labeler CI surface (its intent is equivalently covered by the inlined path filters in `c98cc3a9`).

### Highlights

#### New Features

- **Pi native coding agent** (`84e75ad2` + `40d747c0`): Pi joins the fork as a first-class provider app — provider management, presets and model catalog, native `AGENTS.md` prompt files with a template library, session discovery, session-usage import, and a skills safety path that preserves uninstall state and validates path aliases. Nine new Web commands are registered and route-checked (`check:web-routes` 292/280/0), so Pi behaves identically in the browser and on the desktop. Session usage ships a `v16 -> v17` migration adding the `session_usage_dedup` semantic ledger (plus its `SQL_RESTORE_INDEXES` entry, so canonical SQL restore keeps the index). Ported as purely additive: non-Pi paths were not refactored.
- **Codex Alpha WebSearch** (`bdeaac75`): Codex's Alpha Search endpoint is served through the local proxy via four local aliases, and Claude-side hosted WebSearch is translated end to end — request translation, streaming response bridge, non-streaming responses, usage accounting, and ~30 markdown citation primitives. URL derivation is fail-closed: an opaque full URL that does not literally end in `/responses` is rejected rather than guessed (`strip_suffix`, not a length slice off a normalized parse). Five `MAX_CITATION_DEDUP_*` caps bound quadratic citation-dedup cost on untrusted text (worst observed 6.7 s -> 116 µs), degrading fail-open by skipping only the refinement. One function from the deferred Codex Chat stack proved necessary and was ported as a **private** helper inside its single consumer, with necessity mutation-proven; the deferred stack itself stays absent.
- **Managed OAuth account selection** (`a2e22f33` + the managed-codex transactions of `0455a92c`): providers can bind to a specific managed OAuth account (Codex / Copilot / xAI) instead of "whichever is logged in". Only the account id is persisted — OAuth secrets are stripped on save, and an account flagged `reauth_required` is refused rather than stored (fail-closed, since account binding is an authorization decision). For managed Codex Official cards the selected account's bundle (including `id_token`) is written into the Codex live `auth.json` so its field shape matches a native browser login; that write goes through the fork's managed atomic-write path (ADR 0003), following legitimate symlinks instead of clobbering them. Under local-routing takeover this fork opens **only** managed Official cards — unbound native-login Official cards stay refused at switch time, because the fork resolves the bound account server-side rather than forwarding the client's inbound `Authorization`, so an unbound card has no server-side credential. Upstream's `migrate_legacy_codex_official_managed_binding` startup migration is deliberately **not** ported: upstream deleted it before `v3.20.0`, and it would have moved the user's current provider off the fixed `codex-official` id.

#### Upstream Sync (batches S1–S7)

- **S1 docs/presets/i18n**: the PPIO, JieKou AI and XycAi partner presets added across all supported apps with brand icons and, where supported, endpoint fallbacks; the RunAPI preset migrated to `runapi.host` as its primary domain with `runapi.co` retained as an `endpointCandidates` fallback on Claude Code, Codex and Grok Build; the legacy partner ⭐ badge removed from the main-panel provider cards (partner stars now live only in the preset selector) and the now-unused `officialPartner` translation key dropped from every locale; the `xycai` partner promotion surfaced in every locale; the version bumped to 3.20.0.
- **S2 security/data-integrity**: `fd14f9c4` env-check preflight hang; `d2b070c9` never clobber Codex ChatGPT login (prerequisite for managed OAuth accounts, above); `c8262476` Kimi thinking injection stop; `1f38c838` zhipu `CREDIT_LIMIT` quota; `ccc86298` animate routing activation; `c9fe340b` sync/restore consistency (1012 lines, transactional restore with prepare-once INSERT); `dfb2e523` backup SQL fidelity (1302 lines, multi-row `VALUES` batching, trigger/index ordering). The fork's canonical-schema allow-list, backup/replace lock boundary, 2s JS deadline, 16 MiB heap / 256 KiB stack caps, and SQLite authorizer strictness were all preserved. `3f75bbdf` (StepFun effort inference) was skipped as proven-inapplicable (touches the deferred Codex Chat reasoning stack).
- **S3 pricing/usage**: `bad9c151` DeepSeek V4 peak + Gemini 3.7 Flash; `5602324b` DeepSeek catalog mirror extraction; `7dc0a725` Grok 4.5 cached + Grok 4.6 + DeepSeek alias; `3d126f45` multi-year trend tooltip; `46f19a15` DeepSeek cache-hit tokens. FE-preset ↔ Rust-seed consistency gate passed (no `cost=$0`).
- **S4 codex/provider**: `d1c550ba` drop Goal mode toggle; `6e424fd3` restore 1M context toggle; `a98829ba` IME-safe provider fields (new shared `ImeSafeInput` adopted across Hermes/OpenClaw/OpenCode forms); `f62c854a` cancel stale device login; `b109dcd3` Grok Build Codex copy; `f748f3ac` grokbuild form align; `d9d4a660` macOS IME corruption fix. `0455a92c` multiple follow-login providers split — frontend increments landed here, Rust managed-codex transactions landed with managed OAuth accounts (above). `897ca892` OAuth usage configurable split — frontend + Rust `tray.rs` landed. `40cac1a6` per-model reasoning levels split — catalog data layer (`codex_config.rs` `apply_codex_reasoning_level_override` + spec fields + `types.ts`) landed; transform/UI layer deferred. `d01eab97` (OpenCode Zen reasoning) and `6a7da87c` (grokbuild input token details) skipped as proven-inapplicable (touch the deferred Codex Chat reasoning stack).
- **S5 UI/refactor**: `580a4d7b` Hermes form hierarchy; `ec842156` OpenCode form hierarchy; `c0050623` unify checkbox styles (Radix CheckboxPrimitive → native `<input>`, preserving `onCheckedChange`/`checked` contracts); `5b77da2b` OpenClaw UA section; `95b95da6` OpenClaw model editor; `076c2744` dropdown consolidation; `7e152d75` fuzzy model dropdown (created shared `ModelDropdown.tsx` the fork never carried); `8673e9d8` Claude advanced options align; `bc7f5f41` reduce editor empty space; `7de63227` GrokBuild glass container; `967daa1a` skills SSOT `check_updates` (report missing SSOT dir as an update). `7e5007d5`/`619a592c` (Claude Desktop form) and `390102a2` (OpenCode Go DeepSeek contextWindow) skipped as proven-inapplicable.
- **S6 Windows**: `d4fefefc` startup FOUC (anti-FOUC inline script + themed window background + CSP SHA-256); `de9af49a` Windows CLI detection from registry PATH + standalone installer dirs (adapted to the fork's `services/tool_version.rs`); `3c592d93` WiX Handlebars backslash escape; `c39c9032` WSL atomic-replace fallback (fork's 2s deadline + heap/stack caps preserved).
- **S7 CI**: `c98cc3a9` skip frontend/backend checks when a PR touches neither area (adapted to the fork's 3-job `frontend`/`backend`/`web-server` structure with a `changes` job using `dorny/paths-filter`, SHA-pinned). `36ed280d` i18n labeler glob skipped (fork has no labeler surface). Excluded `ceef0a52` (WSL2 backend tests) and `bef46cd5` (grokBuild exclusion docs) as desktop-only/docs-only.

See **[release notes ->](docs/release-notes/v3.20.0-en.md)** for the per-batch scope, Web adaptations, carry-forward deferrals, and the fork-specific rulings behind the three new features.
